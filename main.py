...
async def transcribe(audio, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    async def produce() -> None:
        try:
            async for partial_text, info in AudioMessageTranscriber.transcribe_stream(
                context, audio
            ):
                if partial_text:
    ...
    async def render_parts(new_parts: list[str], parse_mode: str | None = None) -> None:
        ...
        for index, part in enumerate(new_parts):
            try:
                ...
                logger.debug(
                    f"Telegram flood limit hit, retrying in {error.retry_after}s"
                )
    ...
    markdown_parts = AudioMessageTranscriber.to_markdown(
        {"text": stream["text"], "language": stream["info"].language},
        processing_time,
    )
...
def _get_audio_from_message(message: Message) -> Voice | Audio | VideoNote | None:
    try:
        return message.voice or message.audio or message.video_note or None
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if isinstance(update, Update) and update.effective_message:
        if (
            isinstance(context.error, (BadRequest, FileTooBigError))
            and "too big" in str(context.error).lower()
        ):
            reply_text = (
                "El fichero es demasiado grande para ser descargado (máximo 20 MB)."
            )
        else:
            reply_text = (
...
def main():
    ...
    # Handlers
    app.add_handler(
        CommandHandler("transcribe", transcribe_command, filter_allowed_chats)
    )
